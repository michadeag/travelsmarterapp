 ...
 * Run this to populate the community board with realistic discussions
 * Usage: node backend/scripts/seedCommunity.js
 */
...
async function seedCommunityDiscussions() {
  try {
    ...
    if (postCount > 0) {
      console.log(`ℹ️  Community already has ${postCount} posts. Skipping seed.\n`);
      process.exit(0);
    }
    ...
    for (const post of posts) {
      ...
      postCount++;
      console.log(`✅ Post ${postCount}: "${post.title}"`);
    ...
    console.log(`\n✅ Successfully seeded ${postCount} community posts with ${replyCount} replies!`);
    console.log('\n🎉 Community discussions are now live on the platform!\n');
